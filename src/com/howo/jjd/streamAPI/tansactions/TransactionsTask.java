package com.howo.jjd.streamAPI.tansactions;
...
public class TransactionsTask {
    public static void main(String[] args) {
        ...
        Map<String, Long> stringLongMap = transactionStream
                .collect(Collectors.toMap(tr -> tr.getAccount().getNumber(), s -> s.getSum(), (val1, val2) -> val1 + val2));
        System.out.println(stringLongMap);


        long sumAcc = 0l;
        for (Map.Entry<String, Long> pair: stringLongMap.entrySet()) {
          sumAcc += pair.getValue();
        }
        System.out.println(sumAcc);

        sumAcc = 0l;
        List<Long> list =  stringLongMap.entrySet().stream().map(s -> s.getValue()).collect(Collectors.toList());
        for (Long o : list) {
          sumAcc += o;
        }
        System.out.println(sumAcc);
